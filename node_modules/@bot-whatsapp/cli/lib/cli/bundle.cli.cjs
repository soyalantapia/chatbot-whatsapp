 ...
 * @param {*} templateName
 */
const copyBaseApp$1 = async (fromDir = process.cwd(), toDir = process.cwd()) => {
    const BASEP_APP_PATH_FROM = `${fromDir}`;
    const BASEP_APP_PATH_TO = `${toDir}`;
    await copyFiles(BASEP_APP_PATH_FROM, BASEP_APP_PATH_TO);
};

var createApp = { copyBaseApp: copyBaseApp$1 };

const { red: red$1, yellow: yellow$1, green, bgCyan } = kleur;
const { exec } = require$$1$1;
...
const checkGit$1 = () => {
    return new Promise((resolve, reject) => {
        ...
        exec('git --version', (error) => {
            if (error) {
                console.error(red$1(`🔴 Se require instalar GIT`));
                console.log(``);
                reject('ERROR_GIT');
            } else {
...
var check = { checkNodeVersion: checkNodeVersion$1, checkOs: checkOs$1, checkGit: checkGit$1 };

const prompts = prompts$1;
const { join } = require$$1;
const { yellow, red, cyan, bgMagenta, bgRed } = kleur;
const { existsSync } = require$$0$3;
const { copyBaseApp } = createApp;
const { checkNodeVersion, checkOs, checkGit } = check;
